...
def call_llm_judge(item): 
    ...
    try: 
        ...
        judge_model = os.getenv("DASHSCOPE_JUDGE_MODEL", "qwen-plus")
...
def main():
    ...
    parser.add_argument("--run_id", type=str, default="",
                        help="本次运行的唯一标识（如 qwen-max_20260301_143025），写入 summary 便于对比")
    parser.add_argument("--dataset", type=str, default="gaia", choices=["competition",
                                                                        "gaia", 
    ...
    parser.add_argument("--skip_judge", action="store_true",
                        help="跳过 LLM 评判步骤，仅输出过程统计指标（竞赛数据集无标准答案时使用）")
    ...
    skip_judge = args.skip_judge or (dataset == "competition")
    if dataset in ["gaia", "webwalker", "xbench-deepsearch", "hle"]: 
        ...
        judge_prompt = JUDGE_PROMPT_GAIA
    print(f"Using {dataset} judge prompt ... skip_judge={skip_judge}")
    ...
    if skip_judge:
        # 无标准答案模式：只输出过程统计，不调用 LLM 评判
        print(f"===========")
        print(f"[Skip Judge] Only process statistics are computed.")
        ...
        print(f"[run_id: {run_id}] Statistics saved to {args.restore_result_path}")
        return
    ...
    with open(args.restore_result_path, 'a', encoding='utf-8') as jsonl_file:
        jsonl_file.write(json.dumps(overall_eval_dict, ensure_ascii=False) + '\n')
    print(f"[run_id: {run_id}] Results saved to {args.restore_result_path}")
